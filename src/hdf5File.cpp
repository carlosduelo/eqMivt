...
void hdf5File::read(vmml::vector<3, int> start, vmml::vector<3, int> end, float * data)
{
	if (!_isInit)
		return;
	...
	vmml::vector<3, int> s 		= start; 
	vmml::vector<3, int> e 		= end;
	...
	* memory and display.
	*/
	if ((status = H5Dread(_dataset_id, H5T_IEEE_F32LE/*_datatype*/, memspace, _spaceid, H5P_DEFAULT, data)) < 0)
	{
		LBERROR<<"hdf5: reading data from hyperslab un the file"<<std::endl;
	}
	#ifdef DISK_TIMING
